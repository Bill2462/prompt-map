import { PromptMap } from './prompt_map.js';
...
import { logTaskEnd, logTaskStart, resultSelectedAsSolution,
    resultDisselectedAsSolution, resultDeleted, logSearch} from './log_api.js';

let promptMap = new PromptMap();
...
function onWindowResize() {
    ...
    promptMap.onCanvasResized();
}

function onSearchBtClicked() {
    logSearch(getCookie("prolificPid"), document.getElementById("search-text-area").value,
              document.getElementById("search-field").value);

    promptMap.onSearchBtClicked(document.getElementById("search-text-area").value,
                                document.getElementById("search-field").value);
}

function resetHomeView() {
    promptMap.resetCanvasView();
}

function toggleImagePreview(){
    promptMap.toggleImagePreview();
}

async function init() {
    promptMap.setCanvas(document.getElementById('canvas'));
    await promptMap.fetchMapDataFromApi();

    document.getElementById('canvas').addEventListener('wheel', promptMap.onMouseWheelTurned);
    document.getElementById('canvas').addEventListener('mousedown', promptMap.onMouseDown);
    document.getElementById('canvas').addEventListener('mousemove', promptMap.onMouseMove);
    document.getElementById('canvas').addEventListener('mouseup', promptMap.onMouseUp);
    ...
    document.getElementById("reset-canvas-bt").addEventListener("click", resetHomeView);
    document.getElementById('submit-solution-bt').addEventListener('click', onSubmitSolutionButtonClicked);
    document.getElementById("toggle-img-bt").addEventListener("click", toggleImagePreview);
